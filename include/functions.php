<?php
	...
	{
		setcookie('name', $config['name'], time() + 3600, '/');
		setcookie('rpchost', $config['rpchost'], time() + 3600, '/');
		setcookie('rpcport', $config['rpcport'], time() + 3600, '/');
		setcookie('rpcuser', $config['rpcuser'], time() + 3600, '/');
		setcookie('rpcpassword', $config['rpcpassword'], time() + 3600, '/');
	}
	
	function clear_cookies()
	{
		setcookie('name','', time()-100, '/');
		setcookie('rpchost','', time()-100, '/');
		setcookie('rpcport','', time()-100, '/');
		setcookie('rpcuser','', time()-100, '/');
		setcookie('rpcpassword','', time()-100, '/');
	}
	...
	function read_config()
	{
		...
		if (@$_GET['page']=='home') {
			clear_cookies();
			setcookie('node','', time()-100, '/');
		}
		else if (@$_POST['connection']) {
			$config['name'] = $_POST['name'];
			$config['rpchost'] = $_POST['rpchost'];
			$config['rpcport'] = $_POST['rpcport'];
			$config['rpcuser'] = $_POST['rpcuser'];
			$config['rpcpassword'] = $_POST['rpcpassword'];
			set_cookies($config);
			setcookie('node','', time()-100, '/');
		} 
		else if (@$_POST['connection_test'] || @$_COOKIE['node']) {
			$contents=file_get_contents('include/config.txt');
			$lines=explode("\n", $contents);
			foreach ($lines as $line) {
				...
				if (count($fields)==2) {
					if (is_numeric(strpos($fields[0], '.'))) {
						...
						$config[$parts[1]]=$fields[1];
					} 
				}
			}
			clear_cookies();
			setcookie('node', 'Test', time() + 3600, '/');
		} 
		else if (@$_COOKIE['name'] && @$_COOKIE['rpchost'] && @$_COOKIE['rpcport'] && @$_COOKIE['rpcuser'] && @$_COOKIE['rpcpassword']){
			$config['name'] = $_COOKIE['name'];
			$config['rpchost'] = $_COOKIE['rpchost'];
			$config['rpcport'] = $_COOKIE['rpcport'];
			$config['rpcuser'] = $_COOKIE['rpcuser'];
			$config['rpcpassword'] = $_COOKIE['rpcpassword'];
		}	
		return $config;
	}
	...
	function chain_page_url_html($page=null, $params=array())
	{
		$url='./?';
