...
?>
			...
			<div class="row">
				...
				<div class="col-sm-4"><form method="post" action="./?page=<?php echo html($_GET['page'])?>">
...
<?php
				if ($subscribed) {
?>	
								<td><a href="./?page=<?php echo html($_GET['page'])?>&stream=<?php echo html($stream['createtxid'])?>"><?php echo html($stream['name'])?></a></td>
<?php
				} else {
...
?>
				...
				foreach ($item['publishers'] as $publisher) {
					$link='./?page='.$_GET['page'].'&stream='.$viewstream['createtxid'].'&publisher='.$publisher;
						...
						<tr>
							<th>Key</th>
							<td><a href="./?page=<?php echo html($_GET['page'])?>&stream=<?php echo html($viewstream['createtxid'])?>&key=<?php echo html($item['key'])?>"><?php echo html($item['key'])?></a></td>
						</tr>
						<tr>
							<th>Data</th>
							<td><?php
				...
				if (is_array($file))
					echo '<a href="./download-file?txid='.html($item['txid']).'&vout='.html($item['vout']).'">'.
